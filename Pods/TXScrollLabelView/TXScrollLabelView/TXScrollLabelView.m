...
- (void)setupSubviewsLayout_LeftRight {
    ...
    [self setupLRUDTypeLayoutWithMaxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        labelW = MAX(size.width, weakself.tx_width);
        //开始布局
        weakself.upLabel.frame = CGRectMake(weakself.scrollInset.left, 0, labelW, labelH);
        //由于 TXScrollLabelViewTypeLeftRight\UpDown 类型 X\Y 值均不一样，此处不再block中处理！
        weakself.downLabel.frame = CGRectMake(CGRectGetMaxX(weakself.upLabel.frame) + weakself.scrollSpace, 0, labelW, labelH);
    }];
}
...
- (void)setupSubviewsLayout_UpDown {
    ...
    weakSelf(self);
    [self setupLRUDTypeLayoutWithMaxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        labelH = MAX(size.height, weakself.tx_height);
        weakself.upLabel.frame = CGRectMake(weakself.scrollInset.left, 0, labelW, labelH);
        weakself.downLabel.frame = CGRectMake(weakself.scrollInset.left, CGRectGetMaxY(weakself.upLabel.frame) + self.scrollSpace, labelW, labelH);
    }];
}
...
- (void)updateUpDownScrollLabelLayoutWithText:(NSString *)text labelType:(TXScrollLabelType)type {
    ...
    [self setupLRUDTypeLayoutWithTitle:text maxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        ...
        if (type == TXScrollLabelTypeUp) {
            ...
            weakself.downLabel.frame = CGRectMake(weakself.scrollInset.left, CGRectGetMaxY(weakself.upLabel.frame) + weakself.scrollSpace, labelW, labelH);
        }
    }];
}
