...
- (void)setupSubviewsLayout_LeftRight {
    ...
    CGFloat labelMaxH = self.height;//最大高度
    ...
    [self setupLRUDTypeLayoutWithMaxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        labelW = MAX(size.width, self.width);
        //开始布局
...
- (void)setupSubviewsLayout_UpDown {
    CGFloat labelMaxH = 0;
    CGFloat labelMaxW = self.width - _scrollInset.left - _scrollInset.right;
    CGFloat labelW = labelMaxW;
    __block CGFloat labelH = 0;
    
    [self setupLRUDTypeLayoutWithMaxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        labelH = MAX(size.height, self.height);
        self.upLabel.frame = CGRectMake(_scrollInset.left, 0, labelW, labelH);
...
- (void)setupSubviewsLayout_Flip {
    CGFloat labelW = self.width - _scrollInset.left - _scrollInset.right;
    CGFloat labelX = _scrollInset.left;
    self.upLabel.frame = CGRectMake(labelX, 0, labelW, self.height);
    self.downLabel.frame = CGRectMake(labelX, CGRectGetMaxY(self.upLabel.frame), labelW, self.height);
}
...
- (void)updateScrollingType_LeftRight {
    ...
    if (self.contentOffset.x >= (_scrollInset.left + self.upLabel.width + self.scrollSpace)) {
        /** 更新 Label.text */
        if ((self.contentOffset.x > (_scrollInset.left + self.upLabel.width) - self.width) &&
            self.isArray) {
            [self updateTextForScrollViewWithSEL:@selector(updateLeftRightScrollLabelLayoutWithText:labelType:)];
        }
        [self endup];
...
- (void)updateScrollingType_UpDown {
    if (self.contentOffset.y >= (self.upLabel.height + self.scrollSpace)) {
        /** 更新 Label.text */
        if ((self.contentOffset.y >= (self.upLabel.height)) &&
            self.isArray) {
            [self updateTextForScrollViewWithSEL:@selector(updateUpDownScrollLabelLayoutWithText:labelType:)];
        }
        [self endup];
...
- (void)flipAnimationWithDelay:(NSTimeInterval)delay {
    [UIView transitionWithView:self.upLabel duration:delay * 0.5 options:self.options animations:^{
        self.upLabel.bottom = 0;
        [UIView transitionWithView:self.upLabel duration:delay * 0.5 options:self.options animations:^{
            self.downLabel.top = 0;
        } completion:^(BOOL finished) {
            self.upLabel.top = self.height;
...
-(NSArray *)getSeparatedLinesFromLabel {
    ...
    CGPathAddRect(path, NULL, CGRectMake(0,0,self.upLabel.width,100000));
